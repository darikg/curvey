...
class ApproxMedialAxisBuilder:
    ...
    def finalize(self, close_loops=True) -> Edges:
        """Convert the accumulated vertices into a standalone MA object"""
        if not self.verts:
            return Edges.empty()
        ...
        if close_loops and len(out.edges):
            self.close_loops(out)

        return out

    def close_loops(self, es: Edges) -> None:
        # Look for leaf verts near eachother
        distance = es.point_data["distance"]
        adj = es.to_csgraph(weighted=False, directed=False)
        (leaf_verts,) = where(adj.sum(axis=1) == 1)
        if len(leaf_verts) == 0:
            return

        leaf_points = es.points[leaf_verts]
        ...
        new_edges = leaf_vert_pairs

        # TODO don't modify objects in place here
        es.edges = concatenate([es.edges, new_edges], axis=0)
